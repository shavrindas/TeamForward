...
def event_page2(request, event_id):
    ...
    user_email_cookie = request.COOKIES.get('user_email')
    user = None
    recommended_clothes = []

    if user_email_cookie:
        try:
            user = UserData.objects.get(email=user_email_cookie)
    ...
    for rc in recommended_clothes:
        top_url = rc.top if rc.top.startswith('/media/') else f"/media/{rc.top}"
        bottom_url = rc.bottom if rc.bottom.startswith('/media/') else f"/media/{rc.bottom}"
        outer_url = rc.outer if rc.outer.startswith('/media/') else f"/media/{rc.outer}"
        
        
        print(f"Top URL: {top_url}")
        print(f"Bottom URL: {bottom_url}")
        print(f"Outer URL: {outer_url}")
